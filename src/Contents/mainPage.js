...
function MainPage() {
    const [changeW, setChangeW] = useState("100vw");
    const [changeH, setChangeH] = useState("100vh");
    const [scroll, setScroll] = useState();

    const handleScroll =() =>{
        if(changeW == "100vw" && window.scrollY > (window.innerHeight/3)){
            setChangeH("90vh");
            setChangeW("90vw");
        }
        else if(changeW == "90vw" && window.scrollY > (window.innerHeight/2)){
            setChangeH("70vh");
            setChangeW("70vw");
        }
        else if(changeW == "70vw" && window.scrollY > window.innerHeight){
            setChangeH("50vh");
            setChangeW("50vw");
        }
    }
    ...
    const trigger = new ScrollTrigger({
        ...
        offset:{
            element:{
                x:0,
                y: (trigger, rect, direction)=>{return 0.7}
            }
        }
    }
});
    ...
    useEffect(() => {
        console.log("현재 창 높이 : " + window.innerHeight);
        console.log("현재 창 넓이 : " + window.innerWidth);
    }, [window.innerWidth])


    return <>
        <div>
            <div style={{ width: "100vw", height:(window.innerHeight*3), display:"flex", justifyContent: "center", backgroundColor:"aquamarine", margin:"0px"}}>
                <div style={{ position: "sticky", top: "40px", width: changeW, height:changeH, transition: "0.3s"}}>
                        <img src={img1} style={{width:"100%", height:"100%"}}></img>
                </div>
            </div>

            <article data-trigger style={{ width: "100vw", height: "100vh" }}>
                <h1>Google Docs</h1>
